...
A core principle of C++ templates. If a template argument substitution results in an invalid type or expression, the compiler doesn't throw an errorit simply discards that overload.
...
The line
ostringstream ss;
creates such an object. This object is rst manipulated like a regular stream:
ss << "the answer to everything is " << 42;
...
This is mainly useful when we have a class for which stream serialization has been dened, and for which we want a
...
oating-point values with 3 digits after decimal point:
...
std::generate, std::generate_n and std::transform functions provide a very powerful tool for on-the-y data
...
Arrays
As in the section about reading text les, almost all these considerations may be applied to native arrays. For
...
Automatic arrays must have a compile-time dened size. And the result of a metafunction is a compile-time
...
Version  C++17
With C++17, we get two powerful new tools in our arsenal for solving this problem. The rst is a fold-expression:
template <class... Ts>
void print_all(std::ostream& os, Ts const&... args) {
    ((os << args), ...);
}
...
iterator_category<Iter>::type then lets the compiler select one of the dierent overloads of details::advance.
...
struct has_hash<T, std::void_t< decltype(std::hash<T>()(std::declval<T>())) > >
    : std::true_type
{};
where std::void_t is dened as:
template< class... > using void_t = void;
For detecting if an operator, such as operator< is dened, the syntax is almost the same:
template<class, class = void>
...
1.
The rst declaration of is_pointer_ is the default case, and inherits from std::false_type. The default case
...
remove const and volatile qualiers) and then fall backs to one of the two previous declarations.
