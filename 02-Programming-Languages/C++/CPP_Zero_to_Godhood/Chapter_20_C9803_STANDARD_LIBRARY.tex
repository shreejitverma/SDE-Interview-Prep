...
hold objects - \textbf{Iterators} - Objects to traverse containers -
\textbf{Algorithms} - Functions to manipulate data - \textbf{Function
Objects} - Objects that act like functions
...
\hypertarget{container-characteristics}{%
\section{Container Characteristics}\label{container-characteristics}}
...
unordered\_map & Hash & O(1) avg & O(1) avg & O(1) avg & - & Hash \\
unordered\_set & Hash & O(1) avg & O(1) avg & O(1) avg & - & Hash \\
priority\_queue & Adapter & O(log n) & O(log n) & O(n) & - & Heap \\
...
\hypertarget{comparison-assignment}{%
\subsection{Comparison \& Assignment}\label{comparison-assignment}}
...
\hypertarget{complete-vector-example}{%
\subsection{Complete Vector Example}\label{complete-vector-example}}
...
Fast insertion/deletion at both ends. Like vector but with efficient
...
\hypertarget{deque-vs-vector}{%
\subsection{Deque vs Vector}\label{deque-vs-vector}}

\begin{itemize}
\tightlist
\item
  \textbf{Deque}: Better for push\_front/pop\_front operations
\item
  \textbf{Vector}: Better for push\_back/pop\_back and random access
\end{itemize}
...
\hypertarget{list-specific-operations}{%
\subsection{List-Specific Operations}\label{list-specific-operations}}
...
\hypertarget{map-operations}{%
\subsection{Map Operations}\label{map-operations}}
...
\hypertarget{map-with-custom-comparator}{%
\subsection{Map with Custom
Comparator}\label{map-with-custom-comparator}}
...
\hypertarget{set---sorted-unique-elements}{%
\section{1.5 SET - Sorted Unique
Elements}\label{set---sorted-unique-elements}}
...
\hypertarget{set-with-strings}{%
\subsection{Set with Strings}\label{set-with-strings}}
...
\hypertarget{multiset---allows-duplicates}{%
\subsection{Multiset - Allows
Duplicates}\label{multiset---allows-duplicates}}
...
\hypertarget{multimap---key-value-pairs-with-duplicate-keys}{%
\section{1.6 MULTIMAP - Key-Value Pairs with Duplicate
Keys}\label{multimap---key-value-pairs-with-duplicate-keys}}
...
\hypertarget{unordered_map---hash-based-key-value-pairs}{%
\section{1.7 UNORDERED\_MAP - Hash-Based Key-Value
Pairs}\label{unordered_map---hash-based-key-value-pairs}}

\hypertarget{what-is-unordered-map}{%
\subsection{What is Unordered Map?}\label{what-is-unordered-map}}
...
\hypertarget{unordered-set}{%
\subsection{Unordered Set}\label{unordered-set}}
...
\hypertarget{queue---fifo-first-in-first-out}{%
\section{1.8 QUEUE - FIFO (First In, First
Out)}\label{queue---fifo-first-in-first-out}}

\hypertarget{what-is-queue}{%
\subsection{What is Queue?}\label{what-is-queue}}
...
\hypertarget{queue-example---task-processing}{%
\subsection{Queue Example - Task
Processing}\label{queue-example---task-processing}}
...
\hypertarget{stack---lifo-last-in-first-out}{%
\section{1.9 STACK - LIFO (Last In, First
Out)}\label{stack---lifo-last-in-first-out}}

\hypertarget{what-is-stack}{%
\subsection{What is Stack?}\label{what-is-stack}}
...
\hypertarget{stack-example---balanced-parentheses}{%
\subsection{Stack Example - Balanced
Parentheses}\label{stack-example---balanced-parentheses}}
...
\hypertarget{priority_queue---heap-based-container}{%
\section{1.10 PRIORITY\_QUEUE - Heap-Based
Container}\label{priority_queue---heap-based-container}}

\hypertarget{what-is-priority-queue}{%
\subsection{What is Priority Queue?}\label{what-is-priority-queue}}
...
\hypertarget{priority-queue-with-custom-comparator}{%
\subsection{Priority Queue with Custom
Comparator}\label{priority-queue-with-custom-comparator}}
...
\hypertarget{iterators---deep-dive}{%
\section{ITERATORS - DEEP DIVE}\label{iterators---deep-dive}}

\hypertarget{iterator-categories}{%
\section{Iterator Categories}\label{iterator-categories}}
...
\hypertarget{iterator-types}{%
\subsection{Iterator Types}\label{iterator-types}}
...
\hypertarget{iterator-operations}{%
\subsection{Iterator Operations}\label{iterator-operations}}
...
\hypertarget{reverse-iterators}{%
\subsection{Reverse Iterators}\label{reverse-iterators}}
...
\hypertarget{const-iterators}{%
\subsection{Const Iterators}\label{const-iterators}}
...
\hypertarget{writing-a-custom-iterator}{%
\subsubsection{2. Writing a Custom
Iterator}\label{writing-a-custom-iterator}}

To make a class compatible with STL algorithms (like
\passthrough{\lstinline!std::find!}), you need a conformant iterator.
...
\hypertarget{stream-iterators}{%
\subsubsection{3. Stream Iterators}\label{stream-iterators}}
...
\hypertarget{insert-iterators}{%
\subsubsection{4. Insert Iterators}\label{insert-iterators}}
...
\tightlist
\item
  \passthrough{\lstinline!std::back\_inserter(c)!}: Calls
  \passthrough{\lstinline!c.push\_back(val)!}. (Vector, List, Deque)
\item
  \passthrough{\lstinline!std::front\_inserter(c)!}: Calls
  \passthrough{\lstinline!c.push\_front(val)!}. (List, Deque)
\item
  \passthrough{\lstinline!std::inserter(c, it)!}: Calls
  \passthrough{\lstinline!c.insert(it, val)!}. (Map, Set, List, Vector)
\end{itemize}
...
\hypertarget{algorithms---complete-reference}{%
\section{ALGORITHMS - COMPLETE
REFERENCE}\label{algorithms---complete-reference}}

\hypertarget{c-stl-advanced---extended-reference-algorithms-library}{%
\chapter{C++ STL Advanced - Extended Reference \& Algorithms
Library}\label{c-stl-advanced---extended-reference-algorithms-library}}

\hypertarget{comprehensive-stl-algorithms-reference}{%
\section{COMPREHENSIVE STL ALGORITHMS
REFERENCE}\label{comprehensive-stl-algorithms-reference}}

\hypertarget{all-algorithms-by-category-60-algorithms}{%
\subsection{All Algorithms by Category (60+
Algorithms)}\label{all-algorithms-by-category-60-algorithms}}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\hypertarget{non-modifying-sequence-algorithms}{%
\section{NON-MODIFYING SEQUENCE
ALGORITHMS}\label{non-modifying-sequence-algorithms}}

\hypertarget{find-family}{%
\subsection{1. find Family}\label{find-family}}
...
\hypertarget{count-family}{%
\subsection{2. count Family}\label{count-family}}
...
\subsection{3. mismatch}\label{mismatch}}
...
\hypertarget{equal}{%
\subsection{4. equal}\label{equal}}
...
\hypertarget{search-adjacent_find}{%
\subsection{5. search \& adjacent\_find}\label{search-adjacent_find}}
...
\subsection{6. Logical Operations}\label{logical-operations}}
...
\hypertarget{minmax}{%
\subsection{7. Min/Max}\label{minmax}}
...
\hypertarget{modifying-sequence-algorithms}{%
\section{MODIFYING SEQUENCE
ALGORITHMS}\label{modifying-sequence-algorithms}}

\hypertarget{copy-family}{%
\subsection{1. copy Family}\label{copy-family}}
...
\hypertarget{move-c11}{%
\subsection{2. move (C++11)}\label{move-c11}}
...
\subsection{3. transform}\label{transform}}
...
\hypertarget{fill-generate}{%
\subsection{4. fill \& generate}\label{fill-generate}}
...
\hypertarget{replace}{%
\subsection{5. replace}\label{replace}}
...
\hypertarget{swap-reverse}{%
\subsection{6. swap \& reverse}\label{swap-reverse}}
...
\hypertarget{rotate}{%
\subsection{7. rotate}\label{rotate}}
...
\hypertarget{unique}{%
\subsection{8. unique}\label{unique}}
...
\hypertarget{shuffle-random}{%
\subsection{9. shuffle \& random}\label{shuffle-random}}
...
\hypertarget{remove}{%
\subsection{10. remove}\label{remove}}
...
\hypertarget{sorting-partitioning-algorithms}{%
\section{SORTING \& PARTITIONING
ALGORITHMS}\label{sorting-partitioning-algorithms}}
...
\hypertarget{binary-search-requires-sorted-range}{%
\subsection{Binary Search (requires sorted
range)}\label{binary-search-requires-sorted-range}}
...
\hypertarget{numeric-algorithms}{%
\section{NUMERIC ALGORITHMS}\label{numeric-algorithms}}
...
\hypertarget{set-operations-require-sorted-ranges}{%
\section{SET OPERATIONS (require sorted
ranges)}\label{set-operations-require-sorted-ranges}}
...
\section{HEAP OPERATIONS}\label{heap-operations}}
...
\hypertarget{permutation-algorithms}{%
\section{PERMUTATION ALGORITHMS}\label{permutation-algorithms}}
...
\hypertarget{complete-stl-algorithms-quick-reference}{%
\section{COMPLETE STL ALGORITHMS QUICK
REFERENCE}\label{complete-stl-algorithms-quick-reference}}
...
find\_if & Find matching & Iterator \\
count & Count occurrences & Count \\
equal & Compare ranges & Bool \\
search & Find subsequence & Iterator \\
sort & Sort range & Void \\
binary\_search & Check existence (sorted) & Bool \\
lower\_bound & First \textgreater= value (sorted) & Iterator \\
...
min\_element & Find minimum & Iterator \\
max\_element & Find maximum & Iterator \\
accumulate & Sum/aggregate & Value \\
inner\_product & Dot product & Value \\
set\_union & Union of sets & Iterator \\
set\_intersection & Intersection & Iterator \\
\bottomrule
\end{longtable}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\hypertarget{containers-detailed-operations}{%
\section{CONTAINERS DETAILED
OPERATIONS}\label{containers-detailed-operations}}
...
\hypertarget{map-operations-1}{%
\subsection{Map Operations}\label{map-operations-1}}
...
\hypertarget{algorithm-complexity-cheat-sheet}{%
\section{ALGORITHM COMPLEXITY CHEAT
SHEET}\label{algorithm-complexity-cheat-sheet}}
...
\hypertarget{container-complexity-comparison}{%
\section{CONTAINER COMPLEXITY
COMPARISON}\label{container-complexity-comparison}}
...
\hypertarget{iterator-comparison-table}{%
\section{ITERATOR COMPARISON TABLE}\label{iterator-comparison-table}}
...
\hypertarget{practical-stl-patterns}{%
\section{PRACTICAL STL PATTERNS}\label{practical-stl-patterns}}

\hypertarget{pattern-1-find-remove}{%
\subsection{Pattern 1: Find \& Remove}\label{pattern-1-find-remove}}
...
\hypertarget{pattern-2-filter-copy}{%
\subsection{Pattern 2: Filter \& Copy}\label{pattern-2-filter-copy}}
...
\hypertarget{pattern-3-transform-collect}{%
\subsection{Pattern 3: Transform \&
Collect}\label{pattern-3-transform-collect}}
...
\hypertarget{pattern-4-partition-process}{%
\subsection{Pattern 4: Partition \&
Process}\label{pattern-4-partition-process}}
...
\hypertarget{pattern-5-sort-deduplicate}{%
\subsection{Pattern 5: Sort \&
Deduplicate}\label{pattern-5-sort-deduplicate}}
...
\hypertarget{pattern-6-group-count}{%
\subsection{Pattern 6: Group \& Count}\label{pattern-6-group-count}}
...
\hypertarget{pattern-7-custom-sorting}{%
\subsection{Pattern 7: Custom Sorting}\label{pattern-7-custom-sorting}}
...
\hypertarget{pattern-8-merge-ranges}{%
\subsection{Pattern 8: Merge Ranges}\label{pattern-8-merge-ranges}}
...
\hypertarget{stl-with-lambdas-c11-and-later}{%
\section{STL WITH LAMBDAS (C++11 and
later)}\label{stl-with-lambdas-c11-and-later}}
...
\hypertarget{strings---master-guide}{%
\section{STRINGS - MASTER GUIDE}\label{strings---master-guide}}

\hypertarget{string-basics}{%
\section{4.1 String Basics}\label{string-basics}}
...
\hypertarget{size-and-capacity}{%
\subsection{Size and Capacity}\label{size-and-capacity}}
...
\hypertarget{accessing-characters}{%
\section{4.2 Accessing Characters}\label{accessing-characters}}
...
\section{4.3 String Concatenation}\label{string-concatenation}}
...
\hypertarget{string-searching}{%
\section{4.4 String Searching}\label{string-searching}}
...
\section{4.5 String Manipulation}\label{string-manipulation}}
...
\hypertarget{string-iteration}{%
\section{4.6 String Iteration}\label{string-iteration}}
...
\section{4.7 String Conversion}\label{string-conversion}}
...
\section{4.8 String Comparison}\label{string-comparison}}
...
\hypertarget{string-from-stringstream}{%
\section{4.9 String from Stringstream}\label{string-from-stringstream}}
...
\hypertarget{file-io---complete-coverage}{%
\section{FILE I/O - COMPLETE
COVERAGE}\label{file-io---complete-coverage}}

\hypertarget{file-io-basics}{%
\section{5.1 File I/O Basics}\label{file-io-basics}}
...
\section{5.2 File Operations}\label{file-operations}}

\hypertarget{open-modes}{%
\subsection{Open Modes}\label{open-modes}}
...
\section{5.3 Writing to Files}\label{writing-to-files}}
...
\section{5.4 Reading from Files}\label{reading-from-files}}

\hypertarget{line-by-line}{%
\subsection{Line by Line}\label{line-by-line}}
...
\subsection{Word by Word}\label{word-by-word}}
...
\hypertarget{character-by-character}{%
\subsection{Character by Character}\label{character-by-character}}
...
\hypertarget{specific-format}{%
\subsection{Specific Format}\label{specific-format}}
...
\section{5.5 Binary File I/O}\label{binary-file-io}}
...
\hypertarget{file-position}{%
\section{5.6 File Position}\label{file-position}}
...
\section{5.7 Error Handling}\label{error-handling}}
...
\hypertarget{complete-file-io-example}{%
\section{5.8 Complete File I/O Example}\label{complete-file-io-example}}
...
\hypertarget{function-objects-comparators}{%
\section{FUNCTION OBJECTS \&
COMPARATORS}\label{function-objects-comparators}}

\hypertarget{function-objects-functors}{%
\section{6.1 Function Objects
(Functors)}\label{function-objects-functors}}
...
\hypertarget{standard-comparators}{%
\section{6.2 Standard Comparators}\label{standard-comparators}}
...
\hypertarget{stl-best-practices}{%
\section{STL BEST PRACTICES}\label{stl-best-practices}}

\hypertarget{container-selection}{%
\section{7.1 Container Selection}\label{container-selection}}
...
\section{7.2 Algorithm Selection}\label{algorithm-selection}}
...
\section{7.3 Memory Management}\label{memory-management}}
...
\section{7.4 Performance Tips}\label{performance-tips}}
...
\hypertarget{containers-intro-c98}{%
\subsection{Containers Intro (C++98)}\label{containers-intro-c98}}
