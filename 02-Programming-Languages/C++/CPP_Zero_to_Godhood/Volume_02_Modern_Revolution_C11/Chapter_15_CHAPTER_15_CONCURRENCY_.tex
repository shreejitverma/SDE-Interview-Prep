\hypertarget{chapter-15-concurrency}{%
\section{CHAPTER 15: CONCURRENCY}\label{chapter-15-concurrency}}
...
\subsection{\texorpdfstring{1. Threads
...
\subsection{2. Mutexes \& Locking}\label{mutexes-locking}}
...
\subsection{3. Condition Variables}\label{condition-variables}}
...
\subsection{4. Futures \& Promises}\label{futures-promises}}
...
\subsection{\texorpdfstring{5. Atomics
...
\paragraph{3. Semaphores (C++20)}\label{semaphores-c20}}
...
\paragraph{4. Thread Local Storage
...
\section{Professional Notes: Chapter 80:
...
\section{Professional Notes: Chapter 85:
...
\section{Professional Notes: Chapter 55:
...
while(count--) \{ // Always lock before changing // state guarded by a
mutex and // condition\_variable (a.k.a. ``condvar''). std::lock\_guard
L\{mtx\}; // Push a random int into the queue intq.push(dist(gen)); //
Tell the consumer it has an int cond.notify\_one(); \} // All done. //
...
\section{Chapter 16: Concurrency with
...
\subsection{16.1 Getting Started with
...
\subsubsection{1. Parallel Regions}\label{parallel-regions}}
...
\subsubsection{2. Parallelizing Loops}\label{parallelizing-loops}}
...
\subsection{16.2 Data Sharing
Attributes}\label{data-sharing-attributes}}
...
\hypertarget{professional-notes-openmp-performance}{%
\subsubsection{Professional Notes: OpenMP
Performance}\label{professional-notes-openmp-performance}}

\hypertarget{scheduling-strategies}{%
\paragraph{1. Scheduling Strategies}\label{scheduling-strategies}}

OpenMP provides different ways to distribute loop iterations: *
\passthrough{\lstinline!static!}: Fixed-size chunks assigned at compile
time (Low overhead). * \passthrough{\lstinline!dynamic!}: Chunks
assigned at runtime as threads become free (Better for unbalanced
workloads). * \passthrough{\lstinline!guided!}: Chunks start large and
shrink over time to reduce tail-latency.

\hypertarget{false-sharing-and-padding}{%
\paragraph{2. False Sharing and
Padding}\label{false-sharing-and-padding}}

\textbf{Godhood Warning}: Avoid ``False Sharing,'' where multiple
threads write to different variables that happen to be on the same CPU
cache line. This causes the cache line to be repeatedly invalidated
across cores, drastically reducing performance. * \textbf{Fix}: Pad your
data structures or ensure threads work on data that is spaced apart in
memory.

\hypertarget{thread-affinity}{%
\paragraph{3. Thread Affinity}\label{thread-affinity}}
...
\hypertarget{volume-02-godhood-summary}{%
\section{VOLUME 02: GODHOOD SUMMARY}\label{volume-02-godhood-summary}}

\hypertarget{c11-landmark-library-features}{%
\subsubsection{C++11 LANDMARK LIBRARY
FEATURES}\label{c11-landmark-library-features}}
...
\midrule
\endhead
41 & \textbf{std::unique\_ptr} & Sole-ownership smart pointer with RAII;
replaces raw new/delete &
\passthrough{\lstinline!auto p = std::unique\_ptr<int>(new int(5));!} \\
42 & \textbf{std::shared\_ptr} & Reference-counted shared ownership
smart pointer &
\passthrough{\lstinline!auto p = std::make\_shared<int>(10);!} \\
43 & \textbf{std::weak\_ptr} & Non-owning observer of a shared\_ptr;
breaks reference cycles &
\passthrough{\lstinline!std::weak\_ptr<int> w = p;!} \\
44 & \textbf{std::make\_shared} & Creates a shared\_ptr with a single
combined allocation &
\passthrough{\lstinline!auto p = std::make\_shared<MyClass>(args);!} \\
45 & \textbf{std::move} & Casts an object to an rvalue so move
construction is selected &
\passthrough{\lstinline!std::string b = std::move(a);!} \\
46 & \textbf{std::forward} & Preserves lvalue/rvalue-ness in
forwarding-reference code &
\passthrough{\lstinline!template<class T> void wrap(T\&\& x)\{ use(std::forward<T>(x)); \}!} \\
47 & \textbf{std::thread} & Standard portable threads &
\passthrough{\lstinline!std::thread t([]\{ work(); \}); t.join();!} \\
48 & \textbf{std::mutex} & Basic mutual exclusion primitive &
\passthrough{\lstinline!std::mutex m; std::lock\_guard<std::mutex> lk(m);!} \\
49 & \textbf{std::recursive\_mutex} & Mutex that can be locked multiple
times by same thread &
\passthrough{\lstinline!std::recursive\_mutex m; m.lock(); m.lock();!} \\
50 & \textbf{std::timed\_mutex} & Mutex with try\_lock\_for and
try\_lock\_until &
\passthrough{\lstinline!m.try\_lock\_for(std::chrono::milliseconds(10));!} \\
51 & \textbf{std::lock\_guard} & RAII wrapper that locks on
construction, unlocks on destruction &
\passthrough{\lstinline!std::lock\_guard<std::mutex> lk(m);!} \\
52 & \textbf{std::unique\_lock} & Flexible mutex ownership supporting
deferred/timed locking &
\passthrough{\lstinline!std::unique\_lock<std::mutex> lk(m, std::defer\_lock);!} \\
53 & \textbf{std::condition\_variable} & Allows threads to wait until
notified by another thread &
\passthrough{\lstinline!cv.wait(lock, []\{ return ready; \});!} \\
54 & \textbf{std::atomic} & Atomic types for lock-free access to shared
variables &
\passthrough{\lstinline!std::atomic<int> cnt\{0\}; cnt.fetch\_add(1);!} \\
55 & \textbf{std::future / promise} & Communicate results asynchronously
between threads &
\passthrough{\lstinline!std::promise<int> p; auto f = p.get\_future();!} \\
56 & \textbf{std::async} & Launches a callable asynchronously and
returns a future &
\passthrough{\lstinline!auto f = std::async([]\{ return compute(); \});!} \\
57 & \textbf{std::packaged\_task} & Wraps a callable so its result can
be retrieved via a future &
\passthrough{\lstinline!std::packaged\_task<int()> task(compute);!} \\
58 & \textbf{std::chrono} & Strongly typed clocks, durations, and time
points &
\passthrough{\lstinline!auto t0 = std::chrono::steady\_clock::now();!} \\
59 & \textbf{std::tuple} & Heterogeneous fixed-size collection of values
& \passthrough{\lstinline!auto t = std::make\_tuple(1, 2.5, "hi");!} \\
60 & \textbf{std::tie} & Unpacks a tuple into named variables &
\passthrough{\lstinline!int a; double b; std::tie(a, b) = my\_tuple;!} \\
61 & \textbf{std::array} & Fixed-size STL-style array with zero overhead
& \passthrough{\lstinline!std::array<int,3> a\{\{1,2,3\}\};!} \\
62 & \textbf{std::forward\_list} & Singly linked list optimized for
minimal memory use &
\passthrough{\lstinline!std::forward\_list<int> xs = \{1,2,3\};!} \\
63 & \textbf{std::unordered\_map} & Hash-table based map with average
O(1) lookup &
\passthrough{\lstinline!std::unordered\_map<string,int> mp;!} \\
64 & \textbf{std::unordered\_set} & Hash-table based set &
\passthrough{\lstinline!std::unordered\_set<int> s\{1,2,3\};!} \\
65 & \textbf{Type traits} & Compile-time type property queries for
metaprogramming &
\passthrough{\lstinline!static\_assert(std::is\_integral<int>::value);!} \\
66 & \textbf{std::regex} & Standard regular expression library &
\passthrough{\lstinline!std::regex r("\\\\d+");!} \\
67 & \textbf{std::function} & Type-erased wrapper for any callable &
\passthrough{\lstinline!std::function<int(int)> f;!} \\
68 & \textbf{std::bind} & Binds arguments to a callable &
\passthrough{\lstinline!auto f = std::bind(std::plus<int>\{\}, \_1, 10);!} \\
69 & \textbf{std::begin / end} & Generic free functions for arrays and
containers & \passthrough{\lstinline!auto it = std::begin(arr);!} \\
70 & \textbf{std::to\_string} & Converts numeric types to std::string &
\passthrough{\lstinline!std::string s = std::to\_string(123);!} \\
71 & \textbf{std::stoi / stof} & String to numeric type conversions &
\passthrough{\lstinline!int n = std::stoi("42");!} \\
72 & \textbf{std::initializer\_list} & Sequence of elements for \{\}
initialization &
\passthrough{\lstinline!void f(std::initializer\_list<int> il);!} \\
73 & \textbf{std::exception\_ptr} & Stores and transfers exception
objects between threads &
\passthrough{\lstinline!auto ep = std::current\_exception();!} \\
74 & \textbf{std::random} & Professional engines and distributions &
\passthrough{\lstinline!std::mt19937 rng(42);!} \\
75 & \textbf{std::ratio} & Compile-time rational arithmetic &
\passthrough{\lstinline!using half = std::ratio<1, 2>;!} \\
76 & \textbf{std::enable\_if} & SFINAE helper for conditional templates
&
\passthrough{\lstinline!template<class T, class=std::enable\_if\_t<...>>!} \\
77 & \textbf{std::declval} & Create fake reference for decltype &
\passthrough{\lstinline!decltype(std::declval<T>().member)!} \\
\bottomrule
\end{longtable}

\hypertarget{c11-landmark-language-features-reference}{%
\subsubsection{C++11 LANDMARK LANGUAGE FEATURES
REFERENCE}\label{c11-landmark-language-features-reference}}
...
\midrule
\endhead
1 & \textbf{auto type deduction} & Compiler deduces the type of a
variable from its initializer; reduces verbosity especially with
iterators &
\passthrough{\lstinline!auto x = 42; auto it = v.begin();!} \\
2 & \textbf{decltype} & Queries the declared type of an expression
without evaluating it &
\passthrough{\lstinline!int x = 0; decltype(x) y = 1;!} \\
3 & \textbf{Trailing return types} & Return type is written after the
parameter list using \passthrough{\lstinline!->!}, useful when return
type depends on parameters &
\passthrough{\lstinline!auto add(int a, int b) -> int \{ return a + b; \}!} \\
4 & \textbf{nullptr} & New null pointer constant replacing 0 and NULL;
eliminates overload resolution ambiguity &
\passthrough{\lstinline!int* p = nullptr;!} \\
5 & \textbf{Strongly typed enums} & Scoped enumerations
(\passthrough{\lstinline!enum class!}) that don't leak names; prevent
implicit integer conversion &
\passthrough{\lstinline!enum class Color \{ Red, Green \};!} \\
6 & \textbf{Range-based for loop} & Clean iteration over containers and
arrays without explicit iterators &
\passthrough{\lstinline!for (auto\& x : v) x *= 2;!} \\
7 & \textbf{Lambda expressions} & Anonymous inline function objects with
capture lists &
\passthrough{\lstinline!auto sq = [](int x)\{ return x * x; \};!} \\
8 & \textbf{static\_assert} & Compile-time assertion that stops
compilation with a message if a condition is false &
\passthrough{\lstinline!static\_assert(sizeof(int) >= 4, "msg");!} \\
9 & \textbf{constexpr} & Functions and objects evaluated at compile
time; enables stronger optimization &
\passthrough{\lstinline!constexpr int square(int x)\{ return x*x; \}!} \\
10 & \textbf{Rvalue references} & \passthrough{\lstinline!T\&\&!}
distinguishes temporaries from lvalues; enables move semantics &
\passthrough{\lstinline!void f(std::string\&\& s) \{ /* move */ \}!} \\
11 & \textbf{Move semantics} & Objects can transfer ownership of
resources instead of making expensive deep copies &
\passthrough{\lstinline!std::vector<int> b = std::move(a);!} \\
12 & \textbf{Universal references} & \passthrough{\lstinline!T\&\&!} in
template context can bind to both lvalues and rvalues &
\passthrough{\lstinline!template<class T> void g(T\&\& x);!} \\
13 & \textbf{Variadic templates} & Templates accepting any number of
arguments via parameter packs &
\passthrough{\lstinline!template<class... Ts> void log(Ts... xs) \{\}!} \\
14 & \textbf{Uniform initialization} & Consistent brace initialization
syntax \passthrough{\lstinline!\{\}!} for all types; introduces
\passthrough{\lstinline!initializer\_list!} &
\passthrough{\lstinline!std::vector<int> v\{1,2,3\};!} \\
15 & \textbf{Delegating constructors} & A constructor can call another
constructor in the same class &
\passthrough{\lstinline!A():A(0)\{\}!} \\
16 & \textbf{Inherited constructors} &
\passthrough{\lstinline!using Base::Base!} imports constructors from
base into derived class &
\passthrough{\lstinline!struct D : B \{ using B::B; \};!} \\
17 & \textbf{Defaulted functions} & \passthrough{\lstinline!default!}
asks the compiler to generate standard implementation &
\passthrough{\lstinline!A() = default;!} \\
18 & \textbf{Deleted functions} & \passthrough{\lstinline!delete!}
explicitly forbids a function from being used &
\passthrough{\lstinline!A(const A\&) = delete;!} \\
19 & \textbf{Member initializers} & Data members can be initialized
directly where they are declared &
\passthrough{\lstinline!int x = 10;!} \\
20 & \textbf{override} & Ensures a virtual function in a derived class
actually overrides a base method &
\passthrough{\lstinline!void f() override;!} \\
21 & \textbf{final} & Prevents further overriding or inheritance &
\passthrough{\lstinline!virtual void f() final;!} \\
22 & \textbf{noexcept} & Marks a function as non-throwing; critical for
move operations & \passthrough{\lstinline!void h() noexcept \{\}!} \\
23 & \textbf{Explicit conversion} & Conversion operators that only
trigger when explicitly cast &
\passthrough{\lstinline!explicit operator bool() const;!} \\
24 & \textbf{Ref-qualified members} & Overload based on whether
\passthrough{\lstinline!*this!} is lvalue or rvalue &
\passthrough{\lstinline!void f() \& \{\} void f() \&\& \{\}!} \\
25 & \textbf{Type aliases (using)} & Cleaner alternative to
\passthrough{\lstinline!typedef!}; supports alias templates &
\passthrough{\lstinline!using ll = long long;!} \\
26 & \textbf{Raw string literals} & Strings without backslash escaping
using \passthrough{\lstinline!R"(...)"!} &
\passthrough{\lstinline!std::string s = R"(C:\\temp)";!} \\
27 & \textbf{char16\_t / char32\_t} & Dedicated types for Unicode UTF-16
and UTF-32 code units & \passthrough{\lstinline!char16\_t c = u'a';!} \\
28 & \textbf{User-defined literals} & Custom meaning to literal suffixes
&
\passthrough{\lstinline!long double operator"" \_km(long double x);!} \\
29 & \textbf{{[}{[}attributes{]}{]} syntax} & Standard double-bracket
attribute syntax &
\passthrough{\lstinline![[noreturn]] void fail();!} \\
30 & \textbf{Right-angle bracket fix} & \passthrough{\lstinline!>>!} in
nested templates no longer needs to be written as
\passthrough{\lstinline!> >!} &
\passthrough{\lstinline!std::vector<std::vector<int>> grid;!} \\
31 & \textbf{alignas / alignof} & Control and query alignment
requirements & \passthrough{\lstinline!struct alignas(16) Vec4;!} \\
32 & \textbf{Inline namespaces} & Names are visible from enclosing
namespace; useful for versioning &
\passthrough{\lstinline!inline namespace v1 \{ void f(); \}!} \\
33 & \textbf{Unrestricted unions} & Unions can contain types with
non-trivial members &
\passthrough{\lstinline!union U \{ int i; double d; \};!} \\
34 & \textbf{Extern templates} & Suppresses implicit template
instantiation to reduce compile time &
\passthrough{\lstinline!extern template class std::vector<int>;!} \\
35 & \textbf{std::unique\_ptr} & Sole-ownership smart pointer with RAII;
replaces raw new/delete &
\passthrough{\lstinline!auto p = std::unique\_ptr<int>(new int(5));!} \\
36 & \textbf{std::shared\_ptr} & Reference-counted shared ownership
smart pointer &
\passthrough{\lstinline!auto p = std::make\_shared<int>(10);!} \\
37 & \textbf{std::weak\_ptr} & Non-owning observer of a shared\_ptr;
breaks reference cycles &
\passthrough{\lstinline!std::weak\_ptr<int> w = p;!} \\
38 & \textbf{std::thread} & Standard portable threads &
\passthrough{\lstinline!std::thread t([]\{ work(); \});!} \\
39 & \textbf{std::atomic} & Atomic types for lock-free access to shared
variables & \passthrough{\lstinline!std::atomic<int> cnt\{0\};!} \\
40 & \textbf{std::future / promise} & Communicate results asynchronously
between threads &
\passthrough{\lstinline!std::promise<int> p; auto f = p.get\_future();!} \\
\bottomrule
\end{longtable}

C++11 was the \textbf{Modern Revolution}. It transformed C++ from a
``Better C'' into a high-level, expressive language without sacrificing
a single byte of performance. 1. \textbf{Move Semantics}: The end of
unnecessary copies. 2. \textbf{Smart Pointers}: The end of the ``Memory
Leak Era.'' 3. \textbf{The Threading Model}: Standardized concurrency
for a multi-core world. 4. \textbf{Auto \& Lambdas}: Syntactic sugar
that allowed for more functional and readable code.

\textbf{The Golden Rule of C++11}: Prefer
\passthrough{\lstinline!std::unique\_ptr!} over raw pointers, and use
\passthrough{\lstinline!std::move!} to transfer ownership. You have
transcended the manual memory management of the past.

\hypertarget{volume-03-refinement-generics-c14}{%
\section{VOLUME 03 REFINEMENT GENERICS
