...
\chapter{Professional Notes: Chapter 51:
...
\chapter{Professional Notes: Chapter 56:
...
example, struct Person is deﬁned. It is possible for a person to have a
...
if it is not deﬁned (would be inﬁnity) we instead return the empty
...
cannot diﬀerentiate between a valid 0 and a meaningless 0) and many
types do not have a natural choice for the sentinel value. Optional vs
...
passed by copying. A speciﬁc value of the return type was reserved to
...
that is not an integer would return a pair with an undeﬁned int and a
bool set to false. In this example, John is given two pets, Fluﬀy and
...
one variable safely and eﬃciently. They are basically smart, type-safe
