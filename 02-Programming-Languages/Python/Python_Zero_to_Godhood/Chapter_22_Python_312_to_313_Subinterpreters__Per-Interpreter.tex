...
Before Python 3.12, multi-threaded concurrency in CPython was
constrained by a single process-wide Global Interpreter Lock (GIL).
Although subinterpreters could be spawned via the C-API, they all shared
this single GIL and process-wide global structures, preventing them from
running concurrently across multiple CPU cores.

PEP 684 introduced support for \textbf{isolated subinterpreters}, each
running with its own per-interpreter GIL. This configuration allows a
single OS process to run multiple Python interpreters concurrently,
achieving true multi-core parallel execution.
...
\hypertarget{the-pyinterpreterstate-structure-and-refactoring}{%
\subsubsection{\texorpdfstring{2. The \texttt{PyInterpreterState}
Structure and
Refactoring}{2. The PyInterpreterState Structure and Refactoring}}\label{the-pyinterpreterstate-structure-and-refactoring}}

To achieve GIL isolation, CPython moved all state variables out of
C-static/global variables into fields nested inside the
\passthrough{\lstinline!PyInterpreterState!} structure. This structure
is defined in CPython's internal headers
(\passthrough{\lstinline!Include/internal/pycore\_interp.h!}):
...
\tightlist
\item
  \passthrough{\lstinline!struct \_ceval\_state ceval!}: Wraps the
  evaluation loop structures, including the interpreter's private Global
  Interpreter Lock (\passthrough{\lstinline!ceval.gil!}). This lock is
  completely isolated from other interpreters.
\item
  \passthrough{\lstinline!struct \_gc\_state gc!}: Isolates the
  generational lists and flags of the cyclic Garbage Collector. A
  collection run in one subinterpreter does not block or stop other
  running interpreters.
\item
  \passthrough{\lstinline!PyObject *modules!}: Isolates module
  namespaces. When subinterpreter 1 executes
  \passthrough{\lstinline!import sys!}, it resolves to a different
  dictionary instance than the \passthrough{\lstinline!sys!} module in
  subinterpreter 2.
\end{itemize}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\hypertarget{the-c-api-subinterpreter-initialization-configuration}{%
\subsection{22.2 The C-API Subinterpreter Initialization
Configuration}\label{the-c-api-subinterpreter-initialization-configuration}}

From a C extension, subinterpreters can be configured and created using
the Python C-API. In Python 3.12+, the runtime exposes structure
configurations that specify whether the newly created interpreter should
share the GIL or initialize its own:
...
\hypertarget{cross-interpreter-communication-channels}{%
\subsection{22.3 Cross-Interpreter Communication
Channels}\label{cross-interpreter-communication-channels}}

Because subinterpreters run on separate heaps with private reference
counting states, passing raw object pointers
(\passthrough{\lstinline!PyObject*!}) between them is strictly
forbidden. Sharing pointers would lead to race conditions when multiple
threads increment or decrement reference counts concurrently.

To pass data, the runtime uses a \textbf{shared-nothing} message passing
architecture, serializing objects across isolation boundaries.
...
\tightlist
\item
  \textbf{Serialization}: The sender interpreter extracts the data. It
  uses serialization protocols (such as \passthrough{\lstinline!pickle!}
  or a specialized C-level marshal mechanism) to copy the object's value
  into a raw, C-native memory buffer.
\item
  \textbf{Transmission}: The buffer is queued in a thread-safe global
  memory channel that belongs to the process-global space (external to
  any specific interpreter heap).
\item
  \textbf{Deserialization}: The receiving interpreter pulls the buffer,
  decodes the values, and allocates new objects representing the data on
  its own heap.
\end{enumerate}

\hypertarget{parallel-computation-code-example}{%
\subsubsection{Parallel Computation Code
Example}\label{parallel-computation-code-example}}

In Python 3.12+, you can interact with subinterpreters using the
experimental \passthrough{\lstinline!\_xxsubinterpreters!} module. Below
is a complete script demonstrating true multi-core parallel computation
by distributing work across subinterpreters:
