...
\hypertarget{optimized-order-book-implementation}{%
\subsection{30.2 Optimized Order Book
Implementation}\label{optimized-order-book-implementation}}

To achieve sub-millisecond execution times, the engine uses: 1.
\textbf{\passthrough{\lstinline!\_\_slots\_\_!}}: Avoids class
dictionary allocation overhead for incoming order objects. 2.
\textbf{\passthrough{\lstinline!collections.deque!}}: Provides
\(\mathcal{O}(1)\) insertion and pop times for ordering arrays at
individual price points. 3. \textbf{Dictionary Cache}: Tracks active
order locations by ID to achieve \(\mathcal{O}(1)\) cancellations.
...
\hypertarget{high-throughput-asyncio-server}{%
\subsection{30.3 High-Throughput Asyncio
Server}\label{high-throughput-asyncio-server}}

To receive external market orders, we deploy an optimized asyncio TCP
socket server. The server reads packet strings asynchronously, parsing
and applying them directly to the order book instance.
