---
type: playbook
track: [sde, quant-dev, quant-research, low-latency, ai-eng, distinguished]
level:
status: draft
last_reviewed:
sources: []
---

# 📚 Low Latency Resources

## Vault (Your 14-Module Knowledge Base)
| Module | Link |
|--------|------|
| All 14 Modules | [[14-Low-Latency-Systems/00 Home]] |
| Interview (86KB) | [[14-Low-Latency-Systems/Interview/interview]] |
| Q&A Bank (113KB) | [[14-Low-Latency-Systems/Interview/question-bank-answers]] |
| Thesis Deep Dive (67KB) | [[14-Low-Latency-Systems/Interview/thesis-deep-dive]] |
| C++ Mastery Problems | [[14-Low-Latency-Systems/Interview/Coding-Problems-Low-Latency-CPP-Mastery]] |
| C++ Cheatsheet | [[14-Low-Latency-Systems/Interview/Core-CPP-Low-Latency-Interview-Cheatsheet]] |
| Staff System Design | [[14-Low-Latency-Systems/Interview/Staff-Principal-System-Design-Blueprint]] |
| Performance Engineering | [[12-Performance-Engineering/README\|12-Performance-Engineering]] |
| Seminal LL Papers | [[15-Technical-Whitepapers/10-Seminal-Low-Latency-Systems-Papers]] |

## Books
| Title | Author | Focus |
|-------|--------|-------|
| Systems Performance | Brendan Gregg | Performance analysis |
| C++ Concurrency in Action | Anthony Williams | Modern C++ concurrency |
| Is Parallel Programming Hard? | Paul McKenney | Lock-free, RCU |
| Computer Architecture (Hennessy & Patterson) | | Hardware sympathy |

## External
| Resource | URL |
|----------|-----|
| CppCon Talks (Low Latency) | https://youtube.com/user/CppCon |
| Fedor Pikus Talks | Search CppCon |
| DPDK Documentation | https://doc.dpdk.org |
