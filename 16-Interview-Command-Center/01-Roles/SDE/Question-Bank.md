---
type: playbook
track: [sde, quant-dev, quant-research, low-latency, ai-eng, distinguished]
level:
status: draft
last_reviewed:
sources: []
---

# 🧩 SDE Question Bank

> Curated questions by topic and frequency. `🔥` = Frequently asked. `⭐` = Must-know.

---

## Coding — Arrays & Strings
| # | Problem | Difficulty | Freq | Status |
|---|---------|-----------|------|--------|
| 1 | Two Sum | Easy | 🔥🔥🔥 | ☐ |
| 2 | Best Time to Buy and Sell Stock | Easy | 🔥🔥🔥 | ☐ |
| 3 | Product of Array Except Self | Medium | 🔥🔥🔥 | ☐ |
| 4 | Maximum Subarray (Kadane's) | Medium | 🔥🔥 | ☐ |
| 5 | 3Sum | Medium | 🔥🔥🔥 | ☐ |
| 6 | Container With Most Water | Medium | 🔥🔥 | ☐ |
| 7 | Longest Substring Without Repeating | Medium | 🔥🔥🔥 | ☐ |
| 8 | Minimum Window Substring | Hard | 🔥🔥 | ☐ |
| 9 | Trapping Rain Water | Hard | 🔥🔥 ⭐ | ☐ |
| 10 | Merge Intervals | Medium | 🔥🔥🔥 | ☐ |

## Coding — Trees & Graphs
| # | Problem | Difficulty | Freq | Status |
|---|---------|-----------|------|--------|
| 1 | Invert Binary Tree | Easy | 🔥🔥 | ☐ |
| 2 | Maximum Depth of Binary Tree | Easy | 🔥🔥 | ☐ |
| 3 | Validate BST | Medium | 🔥🔥🔥 | ☐ |
| 4 | Lowest Common Ancestor | Medium | 🔥🔥🔥 ⭐ | ☐ |
| 5 | Serialize/Deserialize Binary Tree | Hard | 🔥🔥 ⭐ | ☐ |
| 6 | Number of Islands | Medium | 🔥🔥🔥 | ☐ |
| 7 | Clone Graph | Medium | 🔥🔥 | ☐ |
| 8 | Course Schedule (Topo Sort) | Medium | 🔥🔥🔥 | ☐ |
| 9 | Word Ladder | Hard | 🔥🔥 | ☐ |
| 10 | Alien Dictionary | Hard | 🔥🔥 ⭐ | ☐ |

## Coding — Dynamic Programming
| # | Problem | Difficulty | Freq | Status |
|---|---------|-----------|------|--------|
| 1 | Climbing Stairs | Easy | 🔥🔥 | ☐ |
| 2 | Coin Change | Medium | 🔥🔥🔥 | ☐ |
| 3 | Longest Increasing Subsequence | Medium | 🔥🔥🔥 ⭐ | ☐ |
| 4 | Word Break | Medium | 🔥🔥🔥 | ☐ |
| 5 | 0/1 Knapsack | Medium | 🔥🔥 ⭐ | ☐ |
| 6 | Edit Distance | Hard | 🔥🔥 ⭐ | ☐ |
| 7 | Longest Common Subsequence | Medium | 🔥🔥 | ☐ |
| 8 | Decode Ways | Medium | 🔥🔥 | ☐ |
| 9 | Regular Expression Matching | Hard | 🔥 | ☐ |
| 10 | Burst Balloons | Hard | 🔥 ⭐ | ☐ |

## System Design — Most Asked
| # | Design Problem | Level | Freq | Status |
|---|---------------|-------|------|--------|
| 1 | URL Shortener (TinyURL) | Mid | 🔥🔥🔥 | ☐ |
| 2 | Rate Limiter | Mid | 🔥🔥🔥 | ☐ |
| 3 | Design Twitter/News Feed | Senior | 🔥🔥🔥 | ☐ |
| 4 | Chat System (WhatsApp) | Senior | 🔥🔥🔥 | ☐ |
| 5 | Distributed Cache (Redis) | Senior | 🔥🔥 | ☐ |
| 6 | Search Autocomplete | Senior | 🔥🔥 | ☐ |
| 7 | Web Crawler | Senior | 🔥🔥 | ☐ |
| 8 | Notification System | Senior | 🔥🔥🔥 | ☐ |
| 9 | YouTube/Netflix Streaming | Staff | 🔥🔥 ⭐ | ☐ |
| 10 | Distributed Task Scheduler | Staff | 🔥🔥 ⭐ | ☐ |
