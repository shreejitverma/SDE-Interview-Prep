# 📊 SDE Skill Matrix — Self-Assessment

> Rate yourself 1-5 for each skill. Update after each study session or interview.
> **1** = No knowledge | **2** = Basic understanding | **3** = Can solve medium problems | **4** = Can solve hard problems | **5** = Can teach it

---

## Data Structures & Algorithms

| Skill | Level | Last Assessed | Notes |
|-------|-------|---------------|-------|
| Arrays & Strings | /5 | | |
| Hash Maps & Sets | /5 | | |
| Linked Lists | /5 | | |
| Stacks & Queues | /5 | | |
| Binary Trees | /5 | | |
| BST & Balanced Trees | /5 | | |
| Heaps / Priority Queues | /5 | | |
| Graphs (BFS/DFS) | /5 | | |
| Topological Sort | /5 | | |
| Union-Find | /5 | | |
| Shortest Path (Dijkstra/Bellman-Ford) | /5 | | |
| Dynamic Programming (1D) | /5 | | |
| Dynamic Programming (2D/Knapsack) | /5 | | |
| Backtracking | /5 | | |
| Sliding Window | /5 | | |
| Two Pointers | /5 | | |
| Binary Search (advanced) | /5 | | |
| Trie | /5 | | |
| Segment Tree / BIT | /5 | | |
| Bit Manipulation | /5 | | |

## System Design

| Skill | Level | Last Assessed | Notes |
|-------|-------|---------------|-------|
| Requirements Gathering | /5 | | |
| API Design | /5 | | |
| Database Selection | /5 | | |
| SQL vs NoSQL Trade-offs | /5 | | |
| Caching Strategies | /5 | | |
| Load Balancing | /5 | | |
| Horizontal Scaling | /5 | | |
| Message Queues | /5 | | |
| Microservices vs Monolith | /5 | | |
| Consistency Models | /5 | | |
| CDN & Edge | /5 | | |
| Rate Limiting | /5 | | |
| Search (Elasticsearch) | /5 | | |
| Streaming (Kafka) | /5 | | |
| Low-Level Design | /5 | | |

## Languages & Tools

| Skill | Level | Last Assessed | Notes |
|-------|-------|---------------|-------|
| Python (fluent coding) | /5 | | |
| C++ (STL, modern features) | /5 | | |
| Java (if applicable) | /5 | | |
| SQL | /5 | | |
| Git | /5 | | |
| Linux CLI | /5 | | |

## Behavioral

| Skill | Level | Last Assessed | Notes |
|-------|-------|---------------|-------|
| STAR Storytelling | /5 | | |
| Quantifying Impact | /5 | | |
| "Why This Company?" | /5 | | |
| Conflict Resolution | /5 | | |
| Leadership Signals | /5 | | |
